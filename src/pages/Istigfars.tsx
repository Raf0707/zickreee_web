...
import istigfarsData from "../data/istigfar.json"; // Импортируем JSON с данными

interface IstigfarsData {
    arabic_dua: string;
    transcript: string;
    translate: string;
}
    ...
    return (
        <div className="min-h-screen bg-[#122428] flex flex-col items-center justify-center overflow-y-auto w-screen">
            ...
            <div className="w-full sm:w-screen space-y-4 px-4 sm:px-12 py-6">
                {istigfarsData.map((item: IstigfarsData, index: number) => (
                    <div
                        key={index}
